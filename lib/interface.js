...
var code = 100;
var CANNOT_CONNECT_TO_CLIENT        = {message: 'CANNOT_CONNECT_TO_CLIENT', code: code++};
...
var validateDirectIBAN = function (iban) {
    return /^XE[0-9]{2}[0-9A-Z]{30}/g.test(iban);
};
...
var getIdentifierAddress = function (identifier, callback) {
...
var transfer = function (identifier, from, to, callback) {
    ...
    if (!validateUserid(from)) {
        log.warn('invalid from ' + from);
        return callback(FROM_IS_INCORRECT);
    }

    var toAddress = validateAddress(to);
    var toDirectIBAN = validateDirectIBAN(to);
    var toIndirectIBAN = validateIndirectIBAN(to);
    var toUserid = validateUserid(to);

    if (toAddress || toDirectIBAN) {
        var address = toAddress ? to : directIBANToAddress(to); 


    } else if (toIndirectIBAN || toUserid) {

        var toIdentifier = toUserid ? identifier : indirectIBANToIdentifier(to);
        getIdentifierAddress(toIdentifier, function (err, address) {
        });
    } else {
        callback(TO_IS_INCORRECT);
    }
