...
var transfer = function (config, identifier, from, to, txPayload, callback) {
    try {
        ...
        if (toAddress || toDirectIBAN) {
            ...
            var hash = web3.eth.contract(abi).at(contractAddress).transfer(from, address, value, txPayload);
            ...
            var hash = web3.eth.contract(abi).at(contractAddress).icapTransfer(from, address, indirectId, value, txPayload);
